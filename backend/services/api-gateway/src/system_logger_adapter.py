...
try:  # prefer shared implementation if available on PYTHONPATH
    ...
    # minimal fallback implementation (writes directly to Redis DB 5)
    try:
    ...
    def _get_client():
        ...
        if _client is not None:
            return _client
        if redis is None:
            logger.debug("system_logger_adapter: redis not available")
            return None
        try:
            _client = redis.Redis(
                host=REDIS_HOST,
            ...
            logger.debug(f"system_logger_adapter: redis connect failed: {e}")
            return None
