"""
AI Orchestrator용 Gemini API 모델 설정 및 초기화 관리 모듈
"""
import os
import logging
...
class GeminiModelConfig:
    ...
    AVAILABLE_MODELS = [
        ...
        "gemini-1.5-flash-latest",   # 빠른 응답 모델 (권장)
        "gemini-pro"                 # 기본 모델 (fallback)
    ]
    
    # 기본 설정값
    DEFAULT_TEMPERATURE = 0.3  # 번역 일관성을 위해 낮춤
    DEFAULT_MODEL = "gemini-1.5-flash-latest"  # 비용 효율적인 모델로 변경
    ...
    def initialize_model(self) -> bool:
        """Gemini 모델 초기화"""
        if not self.api_key:
            logger.warning("GEMINI_API_KEY environment variable not found. AI features will be limited.")
            return False
