...
import ProcessingStatus from '../components/ProcessingStatus'; // 새로 추가
...
const TranslateResult = () => {
  ...
  const [processingStep, setProcessingStep] = useState('EXTRACTING'); // 새로운 상태
...
const TranslateResult = () => {
  ...
  const customSettings = location.state?.customSettings;
  ...
  // AI 에이전트 모니터링 상태
  const [aiAgentData, setAiAgentData] = useState(null);
  ...
  useEffect(() => {
    const fetchAndTranscribe = async () => {
      ...
      try {
        ...
        // 커스텀 모드인 경우 설정 추가
        if (aiMode === 'custom' && customSettings) {
          sttRequestData.custom_settings = customSettings;
        }
        ...
        const currentTaskId = sttRes.data.task_id;
        setTaskId(currentTaskId);

        // 3) WebSocket 연결
        const ws = new WebSocket(
          `${process.env.REACT_APP_WS_BASE_URL}/ws/${currentTaskId}`
        );
        socketRef.current = ws;
        ...
        ws.onmessage = (event) => {
          ...
          if (msg.status) {
            ...
            console.log('📊 상태 업데이트:', msg.status);
          }
          if (msg.progress !== undefined) setProgress(msg.progress);

          // AI 에이전트 모니터링 데이터 처리
          if (msg.ai_agents) {
            console.log('🤖 AI 에이전트 데이터 수신:', msg.ai_agents);
            setAiAgentData(msg.ai_agents);
            setIsAiProcessing(true);
          }

          // AI 처리 시작/완료 상태 감지
          if (msg.status === 'AI_PROCESSING_START' || msg.status === 'AI_PROCESSING' || msg.status === '번역 중...') {
            console.log('🚀 AI 처리 시작 감지');
            setIsAiProcessing(true);
          } else if (msg.status === 'COMPLETED' || msg.status === 'FAILED') {
            console.log('✅ AI 처리 완료 감지');
  ...
  const getModeInfo = () => {
    const modes = {
      fast: { name: '빠른 모드', icon: '⚡', color: 'text-blue-400' },
      standard: { name: '표준 모드', icon: '🎯', color: 'text-green-400' },
      premium: { name: '프리미엄 모드', icon: '✨', color: 'text-purple-400' },
      custom: { name: '커스텀 모드', icon: '🔧', color: 'text-orange-400' }
    };
    return modes[aiMode] || modes.standard;
  };

  return (
    <div className="min-h-screen bg-gray-900 text-white px-4 md:px-12 py-8">
      <h2 className="text-4xl font-extrabold text-center mb-2 text-white drop-shadow">
        🎬 번역 결과
      </h2>
      
      {/* AI 모드 표시 */}
      <div className="text-center mb-6">
        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-800 ${getModeInfo().color}`}>
          {getModeInfo().icon} {getModeInfo().name} 적용 중
        </span>
      </div>
      ...
      <div className="flex flex-col xl:flex-row gap-8">
        ...
        <div className="flex-1 space-y-4">
          ...
          {/* 시스템 모니터링 대시보드 (논문 검증용) */}
          {taskId && (
            <SystemMonitorDashboard taskId={taskId} />
          )}
