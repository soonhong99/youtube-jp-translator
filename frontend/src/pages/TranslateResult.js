...
const TranslateResult = () => {
  // URL에서 전달받은 YouTube 링크
  const location = useLocation();
  const youtubeUrl = location.state?.youtubeUrl;

  // 상태 관리
  const [segments, setSegments] = useState([]);          // 자막 세그먼트
  const [status, setStatus] = useState('STT 진행 중...'); // 현재 처리 상태 텍스트
  const [progress, setProgress] = useState(null);         // 진행률 (0~100)
  const [videoUrl, setVideoUrl] = useState('');           // 유튜브 영상 URL
  const [activeIndex, setActiveIndex] = useState(null);   // 현재 재생 중인 자막 인덱스
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation(); // 영상 이동 방지
                          speak(seg.text);     // TTS 재생
                        }}
